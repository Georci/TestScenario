...
contract ParamCheckModule {
    ...
    event removeSpecificParamRange(address project, bytes4 sig, uint64 index, uint256 min, bool isDiscrete); // 移除某个参数特定的(部分的)拦截范围
    ...
    function setInfo(bytes memory data) external check {
        (address project, bytes4 sig, uint64 index, uint256 min, uint256 max, bool isDiscrete) =
        ...
        if (isDiscrete == true && max == 0) {
            ...
            paramRanges[project][sig][index].discreteBlackParam[arrayIndex] ^= bitMask;
        } else {
    ...
    function removePartialInfo(bytes memory data) external check {
        (bytes4 sig, address project, uint64 index, uint256 min, bool isDiscrete) =
            abi.decode(data, (bytes4, address, uint64, uint256, bool));
        if (isDiscrete == true) {
            uint256 arrayIndex = min / 256;
            uint256 bitPosition = min % 256;
            uint256 bitMask = 1 << bitPosition;
            if (paramRanges[project][sig][index].discreteBlackParam[arrayIndex] & bitMask != 0) {
                paramRanges[project][sig][index].discreteBlackParam[arrayIndex] &= ~bitMask;
            ...
            for (uint64 i = 0; i < length; i++) {
                if (paramRanges[project][sig][index].rangeStart[i] == min) {
                    paramRanges[project][sig][index].rangeStart[i] =
